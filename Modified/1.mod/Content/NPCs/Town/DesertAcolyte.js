...
export class DesertAcolyte extends ModNPC {
  constructor() {
    super();
    this.Texture = "NPCs/Town/DesertAcolyte/" + this.constructor.name;
  }

  SetStaticDefaults() {
    Terraria.Main.npcFrameCount[this.Type] = 25;

    Terraria.ID.NPCID.Sets.DangerDetectRange[this.Type] = 100;
    ...
    new NPCHappiness(this.Type)
      .SetNPCAffection(Terraria.ID.NPCID.Nurse, AffectionLevel.Like)
      .SetNPCAffection(Terraria.ID.NPCID.GoblinTinkerer, AffectionLevel.Hate)
      .SetBiomeAffection(Terraria.ID.BiomeID.Desert, AffectionLevel.Like)
      .SetNPCAffection(Terraria.ID.NPCID.Pirate, AffectionLevel.Dislike)
  }
  ...
  SetBestiary(database, bestiaryEntry) {
    ...
    FlavorText._key = ModLocalization.Translate("Bestiary.DesertAcolyte");
  ...
  CanTownNPCSpawn() {
    return WorldDB.get('Thorium:HasBeenDefeated_TheGrandThunderBird') === true;
  }
  ...
  ModifyNPCHappiness(npc, player, PrimaryPlayerBiome, shopHelper, nearbyNPCsByType) {
    if (player.ZoneDesert) {
      ...
      shopHelper._currentHappiness = ModLocalization.Translate('TownNPCMood.DesertAcolyte.SpecialMoodText');
    }
  }
