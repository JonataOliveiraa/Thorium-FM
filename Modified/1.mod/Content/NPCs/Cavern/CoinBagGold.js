...
export class CoinBagGold extends ModNPC {
    constructor() {
        super();
    ...
    SpawnChance(info) {
        if (Terraria.Main.hardMode && info.CommonEnemy && info.Cavern && info.SpawnTileY < Terraria.Main.maxTilesY - 200 && !info.Water) {
            return 0.0008;
        }
        return 0;
    }
    ...
    PostAI(npc) {
        if (npc.localAI[0] === 1) {
            ...
            if (player && player.active && !player.dead) {
                ...
                npc.velocity = Vector2.new(npc.direction * 3.5, vel.Y);
            }
        }
    }
    ...
    OnHitByPlayer(npc, player, item, damageDone, knockBack) {
        ...
        this.DropCoins(npc, Rand.Next(1, 3));
    }
