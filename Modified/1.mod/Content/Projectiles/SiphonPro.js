...
const SIPHON_COLOR = Color.new(255, 120, 120);

export class SiphonPro extends ModProjectile {
    constructor() {
        super();
    ...
    GetAlpha(proj, color) {
        return Color.Multiply(SIPHON_COLOR, 0.90);
    }
    ...
    AI(proj) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const dustIdx = NewDust(proj.position, proj.width, proj.height, 267, 0, 0, 100, SIPHON_COLOR, 0.75);
    ...
    PreDraw(proj) {
        ...
        for (let k = 0; k < proj.oldPos.Length; k++) {
            ...
            Terraria.Main['void EntitySpriteDraw(Texture2D texture, Vector2 position, Rectangle sourceRectangle, Color color, float rotation, Vector2 origin, float scale, SpriteEffects effects, float worthless)'](
                ...
                Color.Multiply(Color.new(255, 120, 120, 200), 0.2),
