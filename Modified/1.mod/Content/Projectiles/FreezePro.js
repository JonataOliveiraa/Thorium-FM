...
const ICE_COLOR = Color.new(120, 200, 255);
...
export class FreezePro extends ModProjectile {
    ...
    GetAlpha(proj, color) {
        return Color.Multiply(Color.new(120, 200, 255, 0), 0.75);
    }
    ...
    AI(proj) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const dustIdx = NewDust(proj.position, proj.width, proj.height, 135, 0, 0, 100, ICE_COLOR, 0.75);
    ...
    PreDraw(proj) {
        ...
        for (let k = 0; k < proj.oldPos.Length; k++) {
            ...
            Terraria.Main['void EntitySpriteDraw(Texture2D texture, Vector2 position, Rectangle sourceRectangle, Color color, float rotation, Vector2 origin, float scale, SpriteEffects effects, float worthless)'](
                ...
                Color.Multiply(Color.new(120, 200, 255, 200), 0.2),
