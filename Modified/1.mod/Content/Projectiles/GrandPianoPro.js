...
export class GrandPianoPro extends ModProjectile {
    constructor() {
        super();
    ...
    SetStaticDefaults() {
        ...
        this.BlackColor = Color.new(0, 0, 0, 255);
    }
    ...
    GetAlpha(proj, color) {
        return Color.Multiply(Color.Multiply(Color.White, 0.35), proj.Opacity);
    }
    ...
    PreDraw(proj, lightColor) {
        ...
        const getRotItem = proj.oldRot.get_Item;
        ...
        for (let index = length - 1; index > 0; index--) {
            ...
            const color = Color.Multiply(this.GetAlpha(proj, lightColor), (length - index) / length);
            EntitySpriteDraw(texture, drawPos, frame, color, getRotItem(index), drawOrigin, proj.scale, null, 0.0);
        }
        return true;
    }
    ...
    AI(proj) {
        const randomDir = Vector2.Multiply(Vector2.new(Rand.NextFloat(-0.5, 0.5), Rand.NextFloat(-0.5, 0.5)), Vector2.new(20, 40));
        const rotatedOffset = Vector2.RotatedBy(randomDir, Vector2.ToRotation(proj.velocity), Vector2.Zero);
        
        const dust = Terraria.Dust.NewDustPerfect(Vector2.Add(proj.Center, rotatedOffset), 4, null, 130, this.BlackColor, 0.75);
        dust.fadeIn = 1.0;
    ...
    OnKill(proj, timeLeft) {
        ...
        for (let index = 0; index < num; index++) {
            const dust = Terraria.Dust.NewDustDirect(proj.Center, 0, 0, 4, proj.velocity.X * 0.25, proj.velocity.Y * 0.25, 100, this.BlackColor, 1.3);
